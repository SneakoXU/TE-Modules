...
SELECT 'South America' AS Category, avg(lifeexpectancy) AS avg_life_expectancy
FROM country
...
SELECT 'California' AS Category, sum(population) AS total_population
FROM city
...
SELECT 'China' AS Category, sum(population) AS total_population
FROM city
...
SELECT avg(population) AS avg_city_pop_in_USA
FROM city
...
SELECT avg(population) AS avg_city_pop_in_CHN
FROM city
WHERE countrycode = 'CHN';
...
SELECT sum(surfacearea) || ', ' || '"' || continent || '"' AS surface_plus_continent
FROM country
GROUP BY continent
...
FROM country
WHERE population/surfacearea = (SELECT max(population/surfacearea) FROM country);
...
SELECT gnp - gnpold AS gnp_difference, abs(gnpold - gnp) AS absolute_value, name
FROM country
...
SELECT avg(population) AS avg_pop, c.countrycode
FROM city c
WHERE c.countrycode IN (SELECT code FROM country)
